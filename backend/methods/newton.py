...
def newton_roots(expr: sympy.Expr, x0: float, tol: float, niter: int) -> NewtonRoots:
    ...
    while error > tol and fx_old != 0 and fx_prime_old != 0 and iteration < niter:
        ...
        it_data = {'iteration': iteration, 'x': x_new, 'f_x': fx_new, 'f_prime_x': fx_prime_new, 'error': error}
        data.append(it_data)

    if error < tol or fx_old == 0 or fx_prime_old == 0:
        return NewtonRoots(derivative=to_latex(expr_prime), root=x_new, table=data, expression=to_latex(expr))
