...
import { FalseRuleParams, FalseRuleRoots, FalseRuleParamsType, MethodErrorType, ValidationErrorType, FalseRuleRootsType } from '@/lib/types';
...
import { falseRuleRoots } from '@/routers/roots';

addStyles();

type ParamsType = FalseRuleParamsType;

export function FalseRuleRootSolver() {
	const form = useForm<ParamsType>({
		resolver: zodResolver(FalseRuleParams),
	});

	const [solution, setSolution] = useState<FalseRuleRootsType | null>(null);

	async function onSubmit(data: ParamsType) {
		setSolution(null);
		const toastId = toast.loading('Calculating...');
		const result = await falseRuleRoots(data);
		...
		<Card>
			<Form {...form}>
				<form onSubmit={form.handleSubmit(onSubmit)}>
					<CardHeader>
						<CardTitle>Solve False Position roots</CardTitle>
						<CardDescription>
							Use False Position math to solve roots of (almost) any
							function.
						</CardDescription>
					</CardHeader>
					<CardContent>
...
export default FalseRuleRootSolver;
